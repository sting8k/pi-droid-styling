...
`collapsedThinking` accepts `tail` or `label` and defaults to `tail`. When thinking is hidden (`Ctrl+T`), every collapsed row renders `<label> <marker> <tail>` on exactly one line: the label is Pi's own `hiddenThinkingLabel` with its trailing progress dots trimmed, rendered bold and upright, the marker is `▸` while the run is live and `·` once it settles, and the tail is the end of the run's last non-empty line, left-truncated with `…` and coloured by the `collapsedThinkingTailColor` theme extra (default `muted`). When the remaining tail budget falls under 16 columns the tail is dropped and the row is the plain label, and `label` restores today's static label everywhere.
...
- `transparentBackground` should skip page/frame background painting, tool boxes, boxed core message blocks (they share the theme bg funnel), and user-message backgrounds; the editor input, selection, and split-diff semantic diff backgrounds stay opaque.
