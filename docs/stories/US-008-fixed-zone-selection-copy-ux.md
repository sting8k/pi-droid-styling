...
When `fixedUserZone` is enabled, mouse selection in the fixed-zone scrollable/root and sidebar regions should feel closer to native chat selection: drag selects a range, double-click selects a visual word, triple-click selects a visual line, release copies the selected text, and the highlight clears after copy with user-visible feedback.
