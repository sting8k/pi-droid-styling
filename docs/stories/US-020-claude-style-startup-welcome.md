...
New Pi sessions show the PR's rounded, theme-aware welcome banner with live model/cwd, Tips, Context, and What's new content. Resumed sessions do not inject the banner. The compact Pi mark uses the upstream theme-derived gradient without restoring the superseded standalone startup header.
...
- The compact logo uses a gradient derived from the active accent color.
...
- The standalone upstream gradient header is intentionally not restored because PR #17 replaces it with the richer in-chat welcome.
...
| Platform | Width checks at 12, 24, 40, and 96 columns in the smoke script |
