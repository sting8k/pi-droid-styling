...
- P8 custom `hiddenThinkingLabel` verbatim; no `Thinking...` literal in `assistant-prefix.ts`.
- P9 the tail follows only the message passed into `updateContent`; no event or timer source.
- P10 style split: reasonix label italic + `thinkingText`, droid label italics stripped; marker/tail resolve `collapsedThinkingTailColor` and never contain `\x1b[3m`; `#ff0000` recolours only the tail.
