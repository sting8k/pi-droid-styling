...
| US-021 | `presentationStyle` defaults to `droid` and can select `reasonix` for compact user/assistant turns, one-line collapsed tool summaries, and left-rail `Ctrl+O` expanded tool bodies without changing active theme colors; droid remains unchanged | yes | yes | no | no | implemented | `npm run test:reasonix-conversation` covers config scaffold/normalize/fallback, user/assistant rendering, semantic colors, one-row tool summaries, width-bounded expanded rails, collapsed/expanded/error quick-edit behavior, and runtime return to droid; full focused smoke suite; `git diff --check`; `srcwalk review`. |
