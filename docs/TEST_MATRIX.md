...
| US-021 | `presentationStyle` defaults to `droid` and can select `reasonix` for compact turns, colored tool statuses, collapsed header + `└─` metrics rows with a responsive 60%/72-column soft cap, corner-connected `Ctrl+O` bodies, and one spacer row between blocks; droid remains unchanged | yes | yes | no | no | implemented | `npm run test:reasonix-conversation` covers config, semantic statuses, collapsed Bash preview removal, single-row command/error normalization, 24/80/160-column responsive caps, metrics placement, expanded footer/body preservation, legacy-divider removal, user/assistant/tool spacing, quick-edit states, and droid fallback; full focused suite; `git diff --check`; `srcwalk review`. |
