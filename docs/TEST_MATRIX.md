...
| US-021 | `presentationStyle` defaults to `droid` and can select `reasonix` for compact turns, colored tool statuses, collapsed header + `└─` metrics rows, corner-connected `Ctrl+O` bodies, and one spacer row between blocks; droid remains unchanged | yes | yes | no | no | implemented | `npm run test:reasonix-conversation` covers config, semantic statuses, collapsed Bash preview removal, metrics placement/narrow width, expanded footer/body preservation, legacy-divider removal, user/assistant/tool spacing, quick-edit states, and droid fallback; full focused suite; `git diff --check`; `srcwalk review`. |
