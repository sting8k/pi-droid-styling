...
| US-021 | `presentationStyle` defaults to `droid` and can select `reasonix` for compact user/assistant turns, semantic colored tool statuses, one-line collapsed tool summaries, and `└─`-connected `Ctrl+O` expanded bodies; droid remains unchanged | yes | yes | no | no | implemented | `npm run test:reasonix-conversation` covers config, user/assistant rendering, semantic status colors, one-row Bash-like preview collapse, footer metrics, legacy-divider removal, corner-connected expanded bodies, collapsed/expanded/error quick-edit behavior, and runtime return to droid; full focused smoke suite; `git diff --check`; `srcwalk review`. |
