...
| US-020 | `visibleChatTail` defaults to 30, scaffolds/backfills config, accepts 0 as render-all, renders custom tails with an omitted-history indicator, live-reconfigures patched chat containers, and keeps fixed-zone tail=0 windowing bounded | yes | yes | no | no | implemented | `npm run test:tasks-widget-compact` covers config normalization/backfill, tail=N, tail=0, and reconfigure 2→0; `npm run test:user-zone-style` covers fixed-zone + tail=0 windowed root behavior; `git diff --check`; `semantic_review working-tree` |
