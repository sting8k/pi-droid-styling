...
| US-017 | `userZoneStyle` defaults to `gemini` and can select `droid`, `gemini`, or `droid-cli` presets without changing theme format, with Gemini status/input/footer, droid legacy layout, and droid-cli outlined prompt/prompt/status-line layout | yes | yes | no | pending | implemented | `npm run test:user-zone-style` with droid-cli render and inputBox.style override guard smoke; `npm run test:working-message`; `npm run test:theme-extras`; `npm run test:startup-resources`; `git diff --check`; `srcwalk review --limit 10` |
