...
- `transparentBackground: true` skips page/frame background painting, tool boxes, and boxed core message blocks, and resets OSC 11 so the terminal's own default background (wallpaper, blur) shows through. The editor input, selections, and diff accents stay opaque for readability.
