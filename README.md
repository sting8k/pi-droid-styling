...
| `presentationStyle` | `"droid"` \| `"reasonix"` | `"droid"` | Conversation presentation preset. `droid` preserves the existing boxed/divided messages and tool boxes. `reasonix` removes turn cards/dividers, folds collapsed tools into one status/name/subject/metrics row, and renders `Ctrl+O` expanded tool output under a compact left rail while reusing active Pi theme colors. |
