...
| `presentationStyle` | `"droid"` \| `"reasonix"` | `"droid"` | Conversation preset. `droid` preserves existing cards/dividers/tool boxes. `reasonix` aligns user, assistant, and tool text one space after their marker, keeps the theme's assistant marker inline with dim thinking, uses a colored status/name/subject header, keeps Droid-compact tools (`Read`, `Find`, `Search`, `List`, `Write`) on one status/subject/metrics row, puts non-compact metrics on a second `└─` row, soft-caps collapsed rows at 60% of wider terminals (72 columns maximum), expands full output with the semantic-dim connector in column 3 matching status/footer color and body/footer in column 6 using padding-aware width clamps, and keeps one blank row between conversation blocks. |
