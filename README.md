...
| `presentationStyle` | `"droid"` \| `"reasonix"` | `"droid"` | Conversation presentation preset. `droid` preserves the existing boxed/divided messages and tool boxes. `reasonix` uses semantic colored status markers, folds collapsed tools into one name/subject/metrics row, and renders `Ctrl+O` output below a `└─` corner connector without a card or vertical rail. |
