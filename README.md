...
`inputBox.style` controls the input-frame treatment for all `userZoneStyle` presets. Supported values are `auto`, `halfblock`, and `line`; `auto` uses Gemini-style half-block padding normally and falls back to a line frame when `NO_COLOR` is set.
