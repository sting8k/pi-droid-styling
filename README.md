...
| `presentationStyle` | `"droid"` \| `"reasonix"` | `"droid"` | Conversation preset. `droid` preserves existing cards/dividers/tool boxes. `reasonix` uses a colored status/name/subject header, puts collapsed metrics on a second `└─` row, expands full output below the same connector, and keeps one blank row between conversation blocks. |
