...
| `transparentBackground` | `true`, `false` | `false` | Stop painting the page/frame background, tool boxes, and user messages, and reset OSC 11, so the terminal's own background (wallpaper, blur) shows through. The editor, selections, and diff accents stay opaque. Dim/blur is tuned on the terminal side (Ghostty `background-image`, kitty `background_image`, WezTerm `window_background_image`). |
...
- `transparentBackground: true` skips page/frame background painting, tool boxes, boxed core message blocks, and user-message backgrounds, and resets OSC 11 so the terminal's own default background (wallpaper, blur) shows through. The editor input, selections, and diff accents stay opaque for readability.
