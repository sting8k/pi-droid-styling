...
| `transparentBackground` | `true`, `false` | `false` | Stop painting the page/frame background and reset OSC 11 so the terminal's own background (wallpaper, blur) shows through. Tool boxes, the editor, and selections stay opaque. Dim/blur is tuned on the terminal side (Ghostty `background-image`, kitty `background_image`, WezTerm `window_background_image`). |
...
- `transparentBackground: true` skips all page/frame background painting and resets OSC 11 so the terminal's own default background (wallpaper, blur) shows through. Element backgrounds such as tool boxes and the editor stay opaque for readability.
